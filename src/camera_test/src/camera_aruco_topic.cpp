...
class arucoDetect {
public:
    static arucoDetect *GetInstance();// 初始化对象

    void loadStart(ros::NodeHandle &nh);

    // 订阅图片消息
    cv::Mat ArucoImage;
    void getImage(ros::NodeHandle &nh);
...
void arucoDetect::getImage(ros::NodeHandle &nh) {
    sub = nh.subscribe("/k4a/color/color", 1000, &arucoDetect::imageCallback,this);
}
